...
class Plot(object):
    ...
    """
    def __init__(self, txt_width=470, frac_width=0.45, font_size=11,
        ...
        Keyword Arguments:
        txt_width   -- (default 470) from the TeX document \showthe\textwidth
        ...
        self.set_fig_dims(txt_width, frac_width)
...
# Types of plots
###########################################################################
def dd_plot(files, title, xmax=0, ymax=0, lgnd=''):
    """Function for plotting the data distance files
    Keyword Arguments:
    files -- Glob like string of the csv files I want to plot
    title -- Title on plot
    xmax  -- (default 0) range of x-axis
    ymax  -- (default 0) range of y-axis
    lgnd  -- Custom legend
    """

    # Get all the data distance for the lores geocal
    f = sorted(glob.glob(files))

    # Now make plots
    for j in f:
        plt.plot(np.genfromtxt(j, delimiter=',', skip_header=1)[:, 1])

    # This stuff should be handled by the plot class.
    # linewidth=3.0)
